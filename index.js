...
app.get('/', (req, res) => {
    res.render('index', { data: { view: false } })
});
...
app.post("/signup", function (req, res) {
    ...
    User.register(Users, req.body.cpassword, function (err, user) {
        if (err) {
            ...
            res.render('index', { data: { view: false } });//if correct render to login page
...
app.post("/login", function (req, res) {
    ...
    if (!req.body.username) {
        res.render('index', { data: { view: true, msg1: "Username was not given" } })
    } else {
        //if password is not given msg will print
        if (!req.body.password) {
            res.render('index', { data: { view: true, msg: "Password was not given" } })
        } else {
            passport.authenticate('local', function (err, user, info) {
                if (err) {
                    console.log(err)

                    res.render('index', { data: { view: true, msg: err } })
                } else {
                    if (!user) {
                        //if given details are wrong
                        res.render('index', { data: { view: true, msg: "Username or password incorrect " } })//if given details are wrong
                    } else {
